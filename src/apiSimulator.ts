    ...
    return new Promise((resolve, reject) => {
        setTimeout(() => {
            if (Math.random() < 0.8) {
                ...
                reject(new NetworkError("Failed to fetch product catalog: Network timeout"));
            }
        }, 1000);
    });
};
    ...
    return new Promise((resolve, reject) => {
        setTimeout(() => {
            ...
            if (random < 0.75) {
                ...
                reject(new DataError(`Failed to fetch reviews: Product ID ${productId} not found`));
            } else {
                reject(new NetworkError(`Failed to fetch reviews for product ID ${productId}: Server error`));
            }
        }, 1500);
    });
};
    ...
    return new Promise((resolve, reject) => {
        setTimeout(() => {
            if (Math.random() < 0.85) {
                ...
                reject(new NetworkError("Failed fetching sales report: Database connection failed"));
            }
        }, 1000);
    });
};
